...
➩ʚ bank déposer [montant]
➩ʚ bank retirer [montant]
➩ʚ bank solde
➩ʚ bank diamants
➩ʚ bank convertir-diamants [quantité]
➩ʚ bank convertir-argent [montant]
➩ʚ bank transfert-argent [montant] [uid]
➩ʚ bank transfert-diamants [quantité] [uid]
➩ʚ bank interet
➩ʚ bank pret [montant]
➩ʚ bank rembourser
➩ʚ bank gamble [montant]
➩ʚ bank top
━━[🏦 Dëxtëũr Banque 🏦]━━`
    },
    category: "💰 Économie",
  ...
  onStart: async function ({ args, message, event, api, usersData }) {
    ...
    // Chargement des données bancaires existantes si le fichier existe
    try {
      if (fs.existsSync(bankDataPath)) {
        bankData = JSON.parse(fs.readFileSync(bankDataPath, "utf8"));
      }
    } catch (err) {
      console.error("Erreur lors de la lecture du fichier bankData.json:", err);
      return message.reply("❌ Une erreur est survenue avec le système bancaire. Veuillez réessayer plus tard.");
    }

    // Migration des données existantes pour ajouter les diamants si nécessaire
...
Utilisez l'une des commandes suivantes :
➩ʚbank déposer [montant]
➩ʚbank retirer [montant]
➩ ʚbank solde
➩ ʚbank diamants
➩ ʚbank convertir-diamants [quantité]
➩ ʚbank convertir-argent [montant]
➩ ʚbank transfert-argent [montant] [uid]
➩ ʚbank transfert-diamants [quantité] [uid]
➩ ʚbank interet
➩ ʚbank pret [montant]
➩ ʚbank rembourser
➩ ʚbank gamble [montant]
➩ ʚbank top
━━[🏦 Dëxtëũr Banque 🏦]━━`
          );
      }
    } catch (err) {
      console.error("Erreur dans la commande bank:", err);
      return message.reply("❌ Une erreur est survenue lors du traitement de votre requête bancaire.");
    }
  }
};
